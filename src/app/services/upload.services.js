...
export async function UploadWarrantyPDF(file, productId) {
  try {
    ...
    // Check product existence before file persistence
    ...
    // Sanitize file name
    const sanitizedName = (file.name || "document.pdf").replace(
      /[^a-zA-Z0-9.-]/g,
      "_"
    );
    ...
    if (bucketName && hasGcsCreds) {
      try {
        ...
        console.warn(
          "GCS Upload encountered an issue, saving locally as fallback:",
          gcsError.message
        );
        pdfUrl = await saveFileLocally(fileName, buffer);
      }
    } else {
      // Local storage fallback when GCS credentials/bucket are not configured
      pdfUrl = await saveFileLocally(fileName, buffer);
    }
